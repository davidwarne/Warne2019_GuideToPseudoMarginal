#= Test Particle Markov chaim Monte Carlo using Michaelis-Menten Model
#
...
model,species,ν,a,k,x0,T = SelectBCRN("mich-ment")
 Δt = 0.01
σ_obs = 10.0
θ_true = copy(k)
obs_dist = MvNormal(diagm([σ_obs^2,σ_obs^2,σ_obs^2,σ_obs^2]))
g = (X) -> X[:] + rand(obs_dist) 
...
@save "michment.jdl" θ_t1 θ_t2 θ_t3 θ_t4 

pooled_θ = hcat(θ_t1,θ_t2,θ_t3,θ_t4)
Σ_hat = cov(pooled_θ')
println(Σ_hat)
Σ_opt = (((2.38)^2)/3.0)*Σ_hat; 
...
for ii in 1:3
    ...
    @save "michment.jdl" θ_t1 θ_t2 θ_t3 θ_t4 
    R,W,V,S = rnRhat([θ_t1[:,8001:end],θ_t2[:,8001:end],θ_t3[:,8001:end],θ_t4[:,8001:end]])
    println(R)
    println(S)
end
@load "michment.jdl" θ_t1 θ_t2 θ_t3 θ_t4
